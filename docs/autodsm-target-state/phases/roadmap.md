...
- Wire GitHub and Google OAuth (web redirect + Electron modal PKCE) through typed IPC.
- Build sign-in and beta-gate screens.
- Add telemetry batching and feedback widget.

Acceptance: GitHub/Google OAuth sign-in works on web and desktop, beta status gates access, telemetry/feedback write to Supabase without design-system source content.
...
- Build centered T3 Code-style composer and example prompts on `/design-components` (**Create** default; **Browse** keeps registry inspector).
- Infer component name/slug from the user prompt; reserve `src/components/<Name>.tsx` with collision suffixes.
- Client orchestration: `thread.create` → `mergeAutoDsmThreadComponentPaths` → scoped `thread.turn.start` → navigate to three-column agent workspace.
- Refresh registry/preview queries when the agent turn completes so WebContentsView renders the new file after disk writes.

Acceptance: prompt creates a component thread tab in **Components**, streams the agent run in the right rail, indexes/renders the component after ChangeSet apply, and persists conversation history.
