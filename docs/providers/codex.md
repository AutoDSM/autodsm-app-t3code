...
This guide is for people who want to use more than one Codex account in AutoDSM.
...
In AutoDSM Settings, name it something obvious:
...
In AutoDSM Settings, add another Codex provider:
...
AutoDSM shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, AutoDSM treats it
